 ...
 * inputs. Any output nodes not in the map were not transformed.
 */
export class WhitespaceVisitor implements html.Visitor {
  // Whether or not the visitor is currently processing an ICU expansion.
  private inExpansion = false;
  ...
  visitText(text: html.Text, context: SiblingVisitorContext | null): any {
    ...
    // most matters.
    if (this.inExpansion && this.preserveSignificantWhitespace) return text;
